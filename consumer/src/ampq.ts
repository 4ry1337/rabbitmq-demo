...
class RabbitMQ implements Queue {
  ...
  async produce() {
    ...
    produce_channel.assertQueue(response_queue, { durable: true });
    Logger(`CHANNEL CREATED`);
    emitter.on("produce", (msg) => {
      produce_channel.sendToQueue(response_queue, Buffer.from(msg));
      Logger(`Produced Message: ${msg}`);
    });
  }
  ...
  async consume() {
    ...
    consume_channel.assertQueue(request_queue, { durable: true });
    Logger(`CONSUME CHANNEL CREATED`);
    await consume_channel.consume(request_queue, (data) => {
      if (!data) {
